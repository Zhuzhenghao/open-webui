<script lang="ts">
	...
	const cloneHandler = async (tool) => {
		...
		if (_tool) {
			...
			goto(`${BASE_PATH}/workspace/tools/create`);
		}
	};
		...
		<div class=" flex w-full space-x-2">
			...
			<div>
				...
				{:else}
					<a
						class=" px-2 py-2 rounded-xl hover:bg-gray-700/10 dark:hover:bg-gray-100/10 dark:text-gray-300 dark:hover:text-white transition font-medium text-sm flex items-center space-x-1"
						href={`${BASE_PATH}/workspace/tools/create`}
					>
						<Plus className="size-3.5" />
					</a>
				{/if}
		...
		{#each filteredItems as tool}
			...
			>
				<a
					...
					href={`${BASE_PATH}/workspace/tools/edit?id=${encodeURIComponent(tool.id)}`}
				>
				...
				<div class="flex flex-row gap-0.5 self-center">
					...
					{:else}
						...
						<ToolMenu
							editHandler={() => {
								goto(`${BASE_PATH}/workspace/tools/edit?id=${encodeURIComponent(tool.id)}`);
							}}
							shareHandler={() => {
								shareHandler(tool);
							}}
