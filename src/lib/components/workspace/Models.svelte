<script lang="ts">
	...
	const cloneModelHandler = async (model) => {
		...
		goto(`${BASE_PATH}/workspace/models/create`);
	};
		...
		<div class=" flex flex-1 items-center w-full space-x-2">
			...
			<div>
				<a
					...
					href={`${BASE_PATH}/workspace/models/create`}
				>
			...
			>
				<div class="flex gap-4 mt-1 mb-0.5">
					<div class=" w-[44px]">
						...
						>
							<img
								src={model?.meta?.profile_image_url ?? `${BASE_PATH}/static/favicon.png`}
					...
					<a
						...
						href={`${BASE_PATH}/?models=${encodeURIComponent(model.id)}`}
					>
						...
						{:else}
							{#if $user?.role === 'admin' || model.user_id === $user?.id || model.access_control.write.group_ids.some( (wg) => group_ids.includes(wg) )}
								<a
									class="self-center w-fit text-sm px-2 py-2 dark:text-gray-300 dark:hover:text-white hover:bg-black/5 dark:hover:bg-white/5 rounded-xl"
									type="button"
									href={`${BASE_PATH}/workspace/models/edit?id=${encodeURIComponent(model.id)}`}
								>
									<svg
